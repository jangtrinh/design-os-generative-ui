 ...
 */
export function dispatchDesignOSAction(action: ComponentAction, componentId?: string): void {
  if (typeof window !== "undefined" && typeof window.dispatchEvent === "function") {
    const detail: DesignOSActionDetail = {
      action,
      componentId,
      timestamp: Date.now(),
    };
 ...
 */
export function listenDesignOSActions(
  handler: (detail: DesignOSActionDetail, event: CustomEvent<DesignOSActionDetail>) => void
): () => void {
  ...
  const listener = (e: Event) => {
    const customEvent = e as CustomEvent<DesignOSActionDetail>;

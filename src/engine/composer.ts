...
export class DesignOSComposer {
   ...
   */
  async compose(prompt: string, options: Partial<ComposeOptions> = {}): Promise<ComposeResult> {
    ...
    const layoutCriteria: Record<string, string> = {
    ...
    let engineUsed: DecisionTelemetry["engine"] = "deterministic";
    let confidence = 0.85;
    let escalated = false;
  ...
  private enrichSpec(spec: UISpec): UISpec {
  ...
  private assembleSpec(prompt: string, answers: Record<string, any>): UISpec {
    const primaryChoice = answers.primary_widget?.choice || "metric_card";
    const layoutChoice = (answers.layout_topology?.choice as LayoutType) || "stack";

    const getTruthProb = (ans: any): number | undefined => {
      if (!ans) return undefined;
      if (typeof ans.truth_probability === "number") return ans.truth_probability;
    ...
    if (isMarketing) {
      ...
      if (
        (p.includes("giá") || p.includes("gói") || p.includes("pricing") || primaryChoice === "pricing_table") &&
